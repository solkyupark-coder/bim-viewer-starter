...
export function peersFromPresence(state, selfKey) {
  return Object.entries(state || {}).flatMap(([key, metas]) => {
    ...
    return [{
      ...
      surface: m.surface === 'pdf' ? 'pdf' : m.surface === 'dwg' ? 'dwg' : 'model',
...
// DWG/DXF 위에 찍은 핀. 좌표는 2D 도면 좌표라 3D에는 그리지 않습니다.
export function is2dComment(c) {
  return c?.kind === 'dwg';
}

export function commentSurface(c) {
  return isPdfComment(c) ? 'pdf' : is2dComment(c) ? 'dwg' : 'model';
}

export function commentSurfaceLabel(c) {
  const s = commentSurface(c);
  return s === 'pdf' ? '도면' : s === 'dwg' ? '2D' : '모델';
}

export function commentWhere(c) {
  if (isPdfComment(c)) return `도면 p.${c.page || 1}`;
  if (is2dComment(c)) return '2D 도면';
...
export function activityTitle(ev) {
  ...
  const where = surface === 'pdf' ? '도면' : surface === 'dwg' ? '2D' : surface === 'model' ? '모델' : '';
