...
export function schemaGap(msg) {
  return /column|schema cache|does not exist|could not find the/i.test(String(msg || ''));
}
...
/** Displayed (rotated) 0–1 → unrotated page 0–1. pdf.js rotation is clockwise. */
export function pageUnitFromRotated(x, y, deg) {
...
/** Unrotated page 0–1 → displayed (rotated) 0–1. */
export function rotatedFromPageUnit(x, y, deg) {
  const r = pageTurn(deg);
...
export const SAMPLE_PROJECT = '정림 샘플';
export const HLAB_PROJECT = '현대 H LAB';
...
export function lectureFileName(file) {
  ...
  if (kind === 'ifc' && /jeongrim|정림/.test(stem)) return '정림 3D.ifc';
  if ((kind === 'dwg' || kind === 'dxf') && /jeongrim|정림|sample_plan/.test(stem)) {
    return kind === 'dxf' ? '정림 2D.dxf' : '정림 2D.dwg';
  }
  if ((kind === 'pdf' || isPdfName(file?.name)) && /sample_drawing|정림/.test(stem)) return '정림 도면.pdf';
...
export function splitLatest(files, onePerSlot = false) {
  ...
  const order = { ifc: 0, dwg: 1, dxf: 1, pdf: 2 };
...
if (typeof window === 'undefined' && process.argv[1]?.includes('format.js')) {
  ...
  console.assert(isShareableId('a1b2') && !isShareableId('local-1'));
  ...
  console.assert(tree[0].latest.map((f) => lectureFileName(f)).join(',') === '정림 3D.ifc,정림 2D.dwg,정림 도면.pdf');
  ...
  console.assert(dxfRemote[0].latest.map((f) => lectureFileName(f)).join(',') === '정림 3D.ifc,정림 2D.dwg,정림 도면.pdf');
