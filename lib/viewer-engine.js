...
import { is3d, walkPoseFromHit, walkClickIntent, commentHotkeyBlocked, explodeVec, explodePos, verticalExplodeY, clampFov, issueStatus, commentSurface } from './format.js';
...
export class ViewerEngine {
  ...
  commentAtPointer() {
    // 2D(DWG는 SVG, DXF는 dxf-viewer)도 / 키로 핀을 찍습니다. 포인터가 뷰 밖이면 화면 가운데.
    if (this.kind === 'dwg') {
      if (!this.svgEl || !this.svgView) return false;
      return this.commentAtPointer2d(
        (lp) => this.svgPoint(lp.clientX, lp.clientY),
        () => ({ x: this.svgView.x + this.svgView.w / 2, y: this.svgView.y + this.svgView.h / 2 }),
      );
    }
    if (this.kind === 'dxf') {
      if (!this.dxf) return false;
      return this.commentAtPointer2d(
        (lp) => this.dxfPoint(lp.clientX, lp.clientY),
        () => { const c = this.dxf.GetCamera(); return c ? { x: c.position.x, y: c.position.y } : null; },
      );
    }
  ...
  commentAtPointer2d(fromPointer, fallback) {
    const lp = this.lastPointer;
    let p = lp && this.pointerInHost(lp) ? fromPointer(lp) : null;
    if (!p || !Number.isFinite(p.x) || !Number.isFinite(p.y)) p = fallback();
    if (!p) return false;
  ...
  pointerInHost(lp) {
    ...
    return lp.clientX >= r.left && lp.clientX <= r.right && lp.clientY >= r.top && lp.clientY <= r.bottom;
  }

  // 화면 좌표 → dxf-viewer 씬 좌표 (dxf-viewer가 pointer 이벤트에 주는 position과 같은 계산)
  dxfPoint(clientX, clientY) {
    const r = this.dxf.GetCanvas().getBoundingClientRect();
    const v = new THREE.Vector3(((clientX - r.left) / r.width) * 2 - 1, -((clientY - r.top) / r.height) * 2 + 1, 0);
    v.unproject(this.dxf.GetCamera());
    return { x: v.x, y: v.y };
  }
  ...
  async loadDxf(blobUrl) {
    ...
    if (!this.dxfHost) {
      ...
      this.dxfHost.addEventListener('pointermove', (e) => this.rememberPointer(e));
  ...
  drawMarkers() {
    ...
    if (!scene) return;
    const want = this.kind === 'dxf' ? 'dwg' : 'model';
    this.comments.forEach((c, i) => {
      if (commentSurface(c) !== want || c.pos_x == null || issueStatus(c) === 'done') return;
  ...
  drawSvgMarkers() {
    ...
    this.comments.forEach((c, i) => { if (commentSurface(c) === 'dwg' && c.pos_x != null && issueStatus(c) !== 'done') mk(i + 1, c.pos_x, c.pos_y, '#0474E0', i); });
